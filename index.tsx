...
const App = () => {
  ...
  async function fetchEvolutionGroups() {
    ...
    try {
      const response = await fetch(`${config.evolutionApiUrl}/group/fetchAllGroups/${config.instanceName}`, { headers: { 'apikey': config.evolutionApiKey } });
      const data = await response.json();
      const transformed = data.map((g: any) => ({ group_id: g.id, group_name: g.subject || 'Unnamed', member_count: g.size || 0, group_category: 'whatsapp', monitoring_enabled: true, jid: g.id }));
      setGroups(transformed);
      addLog('success', `Matrix Synced.`);
    } catch (err: any) { addLog('error', `Evolution error: ${err.message}`); }
  }
  ...
  async function fetchEvolutionMembers(groupJid: string) {
    ...
    try {
      const response = await fetch(`${config.evolutionApiUrl}/group/participants/${config.instanceName}?groupJid=${groupJid}`, { headers: { 'apikey': config.evolutionApiKey } });
      const participants = await response.json();
      const newContacts = participants.map((p: any) => ({
        member_id: p.id, display_name: p.pushName || p.notify || p.id.split('@')[0], phone_number: p.id.replace('@s.whatsapp.net', '').replace('@c.us', ''), instance_id: parseInt(config.instanceName.includes('EO') ? '2' : '1'), is_eo_member: false, monitoring_enabled: true, enrichment: null, research_log: [], group_ids: [groupJid], is_direct: false
      }));
      setContacts(prev => {
  ...
  const Badge = ({ contact }: { contact: any }) => (
    <div className="flex flex-wrap gap-1 mt-1">
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold ${contact.instance_id === 1 ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-green-50 text-green-600 border border-green-100'}`}>
        {contact.instance_id === 1 ? '📱 SA' : '🌍 EO'}
      </span>
      {contact.is_eo_member && <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 text-purple-600 border border-purple-100 rounded-md text-[10px] font-bold italic">✨ EO - {contact.eo_chapter}</span>}
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              ...
              <select value={selectedInstance} onChange={(e) => setSelectedInstance(e.target.value === 'all' ? 'all' : parseInt(e.target.value))} className="bg-slate-100 border-none rounded-xl px-4 py-2 text-xs font-bold outline-none cursor-pointer">
                <option value="all">📊 Unified</option><option value="1">📱 SA</option><option value="2">🌍 EO</option>
              </select>
            </div>
          </div>
